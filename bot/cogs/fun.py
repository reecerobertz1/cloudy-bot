...
class Fun(commands.Cog):
	...
	@commands.command()
	async def gif(self, ctx,q="random", *, text: str):

		api_key="k0KbPvS0xAo0Hs6XE4tsHhbo6IaS6CpN"
		...
		if text == ():
			...
			# Search Endpoint
				...
				api_response = api_instance.gifs_search_get(api_key, q, limit=50)
				lst = list(api_response.data)
				giff = random.choice(lst)

				emb = discord.Embed(title='GIF provided by GIPHY', color=0x8e6bff)
				...
				# Failed to find a gif!!
				await ctx.reply("Sorry, but I couldn't find a gif of that. :( Try searching for something else!")      
		elif text[0] != ():
			api_response = api_instance.gifs_search_get(api_key, q="  ".join(text[0:len(text)]))
			lst = list(api_response.data)
			giff = random.choice(lst)
			embed = discord.Embed(title='GIF provided by GIPHY', color=0x6effcc)
...
"""    @commands.command()
	async def color(self, ctx, number=None):
		...
		palette = sns.color_palette(ranpal, int(number))
